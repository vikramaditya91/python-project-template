...
PROJECT_NAME = op.basename(op.abspath(op.join(__file__, "..", "..")))
...
def run_command(command, command_inputs=None, cwd=None):
    ...
    if hasattr(command_inputs, '__iter__'):
        for command_input in command_inputs:
            ...
            if sys.version_info[0] >= 3:
                p_result = [x.decode('utf-8') for x in p_result]
            out += p_result[0]
            err += p_result[1]
    else:
...
def install_conda_command():
    ...
    logger.info("Install conda command in current environment...")
...
def create_or_update_root_environment(location):
    """Ensure we get a distribution from which we can create environments.

    :param location
        str, location of the build_env
    """
...
def conda_env():
    ...
    try:
        if not op.exists(tmp_conda):
            ...
            root_interpreter_path = create_or_update_root_environment(tmp_conda)
...
def run_tests(python_prefix):
    ...
    test_results = op.join(OUT_DIR, 'test_results')
...
def execute_on_build_completion(python_prefix):
    """Execute the python tool at the end of build"""
    python_exec = get_python_bin_from_env(env_prefix=python_prefix)

    command = "{0} {1}".format(python_exec,  op.join(PKG_DIR, "project_core",
                                                     PROJECT_NAME, "{}.py".format(PROJECT_NAME)))
    logger.info("Executing the command : {0}".format(command))
    return_code, out, err = run_command(command, cwd=PKG_DIR)
    logger.info("Command {} returned with code {}\noutput: {}\nerror: {}".format(command, return_code, out, err))
...
def main():
    with conda_env() as env_prefix:
        ...
        try:
            ...
            execute_on_build_completion(env_prefix)
            return 0
...
def configure_logging():
    ...
    build_logger.addHandler(stdout_handler)
